...
int Kraken_DecodeMultiArray(const uint8 *src, const uint8 *src_end,
  ...
  // Modified for Reliquary (2026): num_arrays_in_file is masked with 0x3f, so it can reach 63.
  // Newer Oodle Leviathan streams use more than 32 arrays; the original [32] overflowed the stack.
  uint8 *entropy_array_data[64];
  uint32 entropy_array_size[64];
